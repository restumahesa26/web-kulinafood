...
<form class="needs-validation" action="{{ route('process-checkout') }}" method="POST">
                                    ...
                                    @foreach ($product as $item)
                                        @foreach ($item->product as $it)
                                            <div class="media mb-2 border-bottom">
                                                <div class="media-body"> <a href="detail.html">{{ $it->productName }}</a>
                                                    <div class="small text-muted">{{ rupiah($it->price) }}<span
                                                            class="mx-2">|</span> Qty: {{ $item->quantity }} <span
                                                            class="mx-2">|</span> Subtotal:
                                                        {{ rupiah($item->quantity * $it->price) }} </div>
                                                </div>
                                            </div>
                                        @endforeach
                                    @endforeach
    ...
    @foreach ($product as $itemm)
        @foreach ($itemm->product as $i)
            <input type="hidden" name="quantity[]" value="{{ $itemm->quantity }}">
            <input type="hidden" name="product[]" value="{{ $i->id }}">
        @endforeach
        <input type="hidden" name="idee[]" value="{{ $itemm->id }}">
    @endforeach
    <!-- End Cart -->
</form>
@endsection
